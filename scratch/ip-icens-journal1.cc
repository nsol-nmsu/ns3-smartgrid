...
int main (int argc, char *argv[])
{
  ...
  for (int i=0; i<(int)com_nodes.size(); i++) {
	...
	// Subscription messages
        ...
        proHelper.SetAttribute ("Frequency", TimeValue (Seconds (900.0)));
        proApps = proHelper.Install (nodes.Get (com_nodes[i]));
  ...
  for (int i=0; i<(int)agg_nodes.size(); i++) {
	...
	aggHelper.SetAttribute ("Frequency", TimeValue (Seconds (0.1)));
	...
	// AMI messages
        ...
        aggHelper.SetAttribute ("Frequency", TimeValue (Seconds (30)));
  ...
  for (int i=0; i<(int)phy_nodes.size(); i++) {
	if (i < numOfPMUs) {
 		consumerHelper.SetAttribute ("RemoteAddress", AddressValue (InetSocketAddress (Ipv4Address(com_ips[0].c_str()), urgPort)));
  		consumerHelper.SetAttribute ("Frequency", TimeValue (Seconds (480)));
  ...
  for (int i=0; i<(int)phy_nodes.size(); i++) {
	if (i < numOfPMUs) {
		...
		consumerHelper.SetAttribute ("Frequency", TimeValue (Seconds (0.1)));
		consumerHelper.SetAttribute ("Subscription", UintegerValue (0));
  ...
  for (int i=0; i<(int)phy_nodes.size(); i++) {
        if (i >= numOfPMUs) {
                ...
                consumerHelper.SetAttribute ("Frequency", TimeValue (Seconds (30)));
                consumerHelper.SetAttribute ("Subscription", UintegerValue (0));
                consumerHelper.SetAttribute ("PacketSize", UintegerValue (60));
		consumerHelper.SetAttribute ("Offset", UintegerValue (0));
 ...
 for(int itr = 0; itr < (int)phy_nodes.size(); itr++)
  ...
  AnimationInterface anim("anim-ip-scenario1.xml");
...
void ReceivedPacketCallbackCom(uint32_t nodeid, Ptr<Packet> packet, const Address &address, uint32_t localport, uint32_t packetSize, uint32_t subscriptioni, Ipv4Address localip) {
